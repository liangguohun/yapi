...
function setupSql() {
  ...
  let result = userInst.save({
    username: yapi.WEBCONFIG.adminAccount,
    email: yapi.WEBCONFIG.email,
    password: yapi.commons.generatePassword(yapi.WEBCONFIG.password, passsalt),
    passsalt: passsalt,
    role: 'admin',
    ...
    .then(function() {
      ...
      result.then(
        function() {
          ...
          console.log(
            `初始化管理员账号成功,账号名："${yapi.WEBCONFIG.adminAccount}"，密码：${yapi.WEBCONFIG.password}`
          ); // eslint-disable-line
          process.exit(0);
        },
        function(err) {
